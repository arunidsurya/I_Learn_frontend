...
import { handleUpdatePassword } from "../../services/api/adminApi";
...
const ChangePassword: React.FC<Props> = ({ setIsOpen }) => {
  ...
  useEffect(() => {
    ...
    if (storedAdminData) {
      const parseData = JSON.parse(storedAdminData);
      const currentAdmin = parseData;
      
      setEmail(currentAdmin.admin.email);
      setSuccessMessage("");
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const res = await handleUpdatePassword(email, oldPassword, newPassword);
      
      if (res?.data.success) {
        setUnSuccessMessage("");
        setSuccessMessage(res.data.admin.message);
      } else {
        setSuccessMessage("");
